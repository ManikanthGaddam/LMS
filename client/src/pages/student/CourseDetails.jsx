...
const CourseDetails = () => {
  ...
  return courseData ? (
    <>
      <div className='flex md:flex-row flex-col-reverse gap-10 relative items-start justify-between md:px-36 px-8 md:pt-30 pt-20 text-left min-h-screen'>
        <div className='absolute top-0 left-0 w-full h-100vh -z-10 bg-gradient-to-b from-cyan-100/70'></div>
        ...
        <div className='max-w-xl z-10 text-gray-500'>
          ...
          <div className="flex items-center space-x-1 mt-1 pt-3 pb-1 text-sm">
            <p className="text-yellow-500 font-semibold text-sm">{calculateRating(courseData)}</p>
            <div className="flex">
              {[...Array(5)].map((_, i) => (
                <img
                  key={i}
            ...
            <p className="text-blue-600 text-xs">({courseData.courseRatings.length} {courseData.courseRatings.length > 1 ? 'ratings' : 'rating'})</p>
            <p>{courseData.enrolledStudents.length} {courseData.enrolledStudents.length > 1 ? 'students' : 'student'}</p>
          </div>

          <p className='text-sm'>Course by <span className='text-blue-600 underline'>Manikanth</span></p>
          <div className='pt-8 text-gray-800'>
            ...
            <div className='py-20 text-sm md:text-base'>
              ...
              <p className="pt-3 text-gray-700 leading-relaxed md:text-base text-sm first-letter:text-4xl first-letter:font-bold first-letter:text-blue-600 first-letter:mr-2">
                <span dangerouslySetInnerHTML={{ __html: courseData.courseDescription }}></span>
              </p>
        ...
        {/* right column */}
        <div className='z-10 bg-white rounded-t md:rounded-none overflow-hidden min-w-[300px] sm:min-w-[420px] shadow-lg'>
          {
            playerData ? 
